...
Additionally, for more recent elections from 2014 to 2022, we can download shapefiles from the [US Census Bureau](https://www.census.gov/geographies/mapping-files/time-series/geo/tiger-line-file.html). The following code will automatically download these congressional district shapefiles.
...
After gathering the house seat change from 1976 to 2022, I found that `{r} no_change` states had no seat change (`{r} str_to_title(no_change_seats)`), while `{r} 50 - no_change` states did. Of these `{r} 50 - no_change` states, `{r} gain_seats` states gained seats while `{r} lost_seats` states lost seats. Texas gained the most seats from 1976 with `{r} gained_most_seats` gained seats. While New York lost the most seats from 1976 with `{r} lost_most_seats` lost seats.
