...
For this question, I decided to filter my data table to include only movies that exceeded the 3rd quartile number of votes `{r} quantile(movies_ratings_success$numVotes, 0.75)` with a success rate lower than the 1st quartile success indices `{r} quantile(movies_ratings_success$success, 0.25)`.

The lowest three were: Nyay: The Justice, 2025 - The World Enslaved by a Virus, and Santa and the Ice Cream Bunny. All three of these movies scored low on the success and had low average ratings from IMDb. There was limited information on the movie Nyay: The Justice, but from what was available, it seemed that this movie didn’t perform well at all. Rotten Tomatoes provided some additional insight on the two remaining movies. Santa and the Ice Cream Bunny scored a 12% on the popcornmeter (audience vote metric) out of a scale of 100 and The World Enslaved by a Virus scored a 0% on the popcornmeter from Rotten Tomatoes.
...
steven_spielberg_works <- left_join(steven_spielberg_basics, TITLE_PRINCIPALS, by = 'nconst') |>
  ...
  select(tconst, primaryTitle, startYear, averageRating, numVotes, success) |>
  drop_na() |>
  unique()
...
```
Then after further adjustment and considering various numeric thresholds for my success metric, I decided on a success threshold of v = 0.2.
